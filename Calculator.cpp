...
class Token_stream {
     ...
     public:
            Token_stream();        // make a Token_stream(); make a Token_stream that reads from cin
            Token get();           // get a Token (get() is defined elsewhere)
            void putback(Token t); // put a token back
            void ignore(char c);   // discard tokens up to including a c
...
Token Token_stream::get(){
     ...
     case '=':
     case '!':
             return Token(ch); // let each character itself
     ...
     case '9':
             {
                cin.putback(ch);  // put a digit back into the input stream
                double val;
...
//we are not implementing this function to the calculator yet 
...
double declaration(){
     ...
     string var_name = t.name; // i think we dont need var_name
     Token t2 = ts.get();

     if (t2.kind != '=') error("= missing in declaration of " , var_name);

     double x = expression();
     define_name(var_name , x);
     return x;
...
void calculate(){
     ...
     while(cin){
          try{
               ...
               cerr << e.what() << endl;     // write error messsage
               clean_up_mess();
